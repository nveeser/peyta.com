...
class Ghost:
    ...
    def bulk_transcode(self, source, bulk_root, backup_root):
        source = os.path.abspath(source)
        for inpath in _yield_files(source, EXT_VIDEOS):
            if _extension(inpath) == 'mp4' and self.verbose:
    ...
    def process(self, indir, dest, base, extensions=EXT_ALL):        
        ...
        files = [ f for f in files if include_f(f) ]

        for inpath in self.prefix(files):
